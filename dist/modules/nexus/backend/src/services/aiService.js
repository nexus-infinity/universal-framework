            ...
            switch (_a.label) {
                case 0: return [4 /*yield*/, openai.chat.completions.create({
                        ...
                        messages: [
                            ...
                            { role: "user", content: "Generate tags for this file content: ".concat(fileContent) }
                        ],
                        max_tokens: 50
                    })];
                case 1:
                    response = _a.sent();
                    return [2 /*return*/, response.choices[0].message.content.split(',').map(function (tag) { return tag.trim(); })];
            }
        });
    });
}
function analyzeRelationships(files) {
    return __awaiter(this, void 0, void 0, function () {
        var fileDescriptions, response;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    fileDescriptions = files.map(function (file) { return "".concat(file.name, ": ").concat(file.content); }).join('\n');
                    return [4 /*yield*/, openai.chat.completions.create({
                            model: "gpt-4",
                            messages: [
                                { role: "system", content: "You are a file relationship analyzer. Analyze the relationships between the given files." },
                                { role: "user", content: "Analyze relationships between these files:\n".concat(fileDescriptions) }
                            ],
                            max_tokens: 200
                        })];
                case 1:
                    response = _a.sent();
                    return [2 /*return*/, JSON.parse(response.choices[0].message.content)];
            }
        });
    });
}
